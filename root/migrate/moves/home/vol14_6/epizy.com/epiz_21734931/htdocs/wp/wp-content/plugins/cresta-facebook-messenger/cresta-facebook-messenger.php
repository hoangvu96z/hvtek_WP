...
define( 'CRESTA_FACEBOOK_MESSENGER_PLUGIN_VERSION', '1.1.5' );
...
function cresta_facebook_messenger_menu() {
	...
	$cresta_facebook_options_page = add_options_page( esc_html__( 'Cresta Facebook Messenger Settings'), 'Cresta Facebook Messenger', 'manage_options', 'cresta-facebook-messenger.php', 'cresta_facebook_messenger_option', 81 );
}
...
function crestafacebookplugin_front_enqueue_scripts() {
	wp_enqueue_style( 'cresta-facebook-messenger-front-style', plugins_url('css/cresta-facebook-messenger-front-css.css',__FILE__), array(), CRESTA_FACEBOOK_MESSENGER_PLUGIN_VERSION);
}
...
function crestafacebookplugin_admin_enqueue_scripts( $hook ) {
	...
	if ( $hook == $cresta_facebook_options_page ) {
		wp_enqueue_style( 'cresta-facebook-messenger-admin-style', plugins_url('css/cresta-facebook-messenger-admin-css.css',__FILE__), array(), CRESTA_FACEBOOK_MESSENGER_PLUGIN_VERSION);
	}
}
...
function register_cresta_facebook_messenger_settings() {
	...
	$cfm_options_arr = array(
		...
		'cresta_facebook_messenger_box_text' => 'Cresta Facebook Messenger',
		'cresta_facebook_messenger_hide_cover' => '',
		'cresta_facebook_messenger_small_header' => '',	
		'cresta_facebook_messenger_show_facepile' => '',
		'cresta_facebook_messenger_tabs' => 'messages',
		'cresta_facebook_messenger_box_language' => 'en_US',
		'cresta_facebook_messenger_zindex' => '999',
		'cresta_facebook_messenger_show_floating_box' => '1',	
		...
		'cresta_facebook_messenger_click_to_close' => '',
...
function cresta_facebook_messenger_shortcode( $atts ) {
	...
	return '<div id="fb-root"></div>
			<script async defer src="https://connect.facebook.net/'.esc_attr(trim($facebook_language)).'/sdk.js#xfbml=1&version=v3.2&autoLogAppEvents=1"></script>
			<script>
			(function($) {
			"use strict";
...
function add_cresta_facebook_messenger_box() {
	...
	if ($facebook_show_floating_box == 1 && cresta_facebook_messenger_show_floating()) {
		...
		<div id="cfmexist"></div>
		<div id="fb-root"></div>
		<script async defer src="https://connect.facebook.net/<?php echo esc_attr(trim($facebook_language)); ?>/sdk.js#xfbml=1&version=v3.2&autoLogAppEvents=1"></script>
		<script>
		(function($) {
		"use strict";
			$(document).ready(function() {
...
function cresta_facebook_messenger_option() {
	...
	<div class="wrap">
		...
		<a class="crestaButtonUpgrade" href="https://crestaproject.com/downloads/cresta-facebook-messenger/" target="_blank" title="<?php esc_attr_e('See Details: Cresta Facebook Messenger PRO', 'cresta-facebook-messenger'); ?>"><span class="dashicons dashicons-megaphone"></span><?php esc_html_e('Upgrade to PRO version!', 'cresta-facebook-messenger'); ?></a>
		...
		<div id="poststuff">
			...
			<form method="post" action="options.php">
				...
				<table class="form-table">
					...
					<tbody>
						<tr valign="top">
							<th scope="row"><?php esc_html_e( 'Your Facebook Page ID:', 'cresta-facebook-messenger' ); ?></th>
							<td>
								...
								printf( esc_html__( 'The system works only with Facebook pages (no profiles or groups). Use %1$s this website %2$s to find the ID of your Facebook page.', 'cresta-facebook-messenger' ), '<a href="https://findmyfbid.com/" target="_blank">', '</a>' );
								?>
								</span>
							</td>
						</tr>
...
function crestafacebookmessenger_options_validate($input) {
	if($input['cresta_facebook_messenger_tabs'] != '' && is_array($input['cresta_facebook_messenger_tabs'])) {
		...
		$input['cresta_facebook_messenger_tabs'] = wp_filter_nohtml_kses($tabs_to_show); 
	} else {
		$input['cresta_facebook_messenger_tabs'] = 'messages'; 
	}
	$input['cresta_facebook_messenger_page_url'] = sanitize_text_field($input['cresta_facebook_messenger_page_url']);
	$input['cresta_facebook_messenger_box_text'] = sanitize_text_field($input['cresta_facebook_messenger_box_text']);
	$input['cresta_facebook_messenger_hide_cover'] = wp_filter_nohtml_kses($input['cresta_facebook_messenger_hide_cover']);
	$input['cresta_facebook_messenger_small_header'] = wp_filter_nohtml_kses($input['cresta_facebook_messenger_small_header']);
	$input['cresta_facebook_messenger_show_facepile'] = wp_filter_nohtml_kses($input['cresta_facebook_messenger_show_facepile']);
	$input['cresta_facebook_messenger_box_language'] = sanitize_text_field(wp_unslash($input['cresta_facebook_messenger_box_language']));
	$input['cresta_facebook_messenger_zindex'] = sanitize_text_field(absint($input['cresta_facebook_messenger_zindex']));
	$input['cresta_facebook_messenger_show_floating_box'] = wp_filter_nohtml_kses($input['cresta_facebook_messenger_show_floating_box']);
	$input['cresta_facebook_messenger_mobile_option'] = sanitize_text_field(wp_unslash($input['cresta_facebook_messenger_mobile_option']));
	$input['cresta_facebook_messenger_show_option'] = sanitize_text_field(wp_unslash($input['cresta_facebook_messenger_show_option']));
	$input['cresta_facebook_messenger_click_to_close'] = wp_filter_nohtml_kses($input['cresta_facebook_messenger_click_to_close']);
	$input['cresta_facebook_messenger_what_icon'] = sanitize_text_field(wp_unslash($input['cresta_facebook_messenger_what_icon']));
	if($input['cresta_facebook_messenger_selected_page'] != '' && is_array($input['cresta_facebook_messenger_selected_page'])) {
		...
		$input['cresta_facebook_messenger_selected_page'] = wp_filter_nohtml_kses($box_show_on); 
	} else {
		$input['cresta_facebook_messenger_selected_page'] = 'homepage,blogpage,post,page'; 
	}
	return $input;
}
?>